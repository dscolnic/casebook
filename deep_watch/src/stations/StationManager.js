 ...
 * station (fires on 'interact:station'), owns its lifecycle, and closes it on Esc
 * / "step back". While a station is open, player movement is disabled and the
 * pointer is unlocked so the DOM console is usable.
 */
const STATIONS = {
...
export class StationManager {
  constructor({ eventBus, state, save }) {
    this.bus = eventBus;
    this.ctx = { bus: eventBus, state, save };
    ...
    this.bus.on('interact:station', (rec) => this.open(rec.data.station));
