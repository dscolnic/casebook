// world.js — Cerro Vela Survey Operations, wired to the engine's world contract.
//
// The geometry is themes/bring_them_home/world.js: one large volume with the floor
// stepped down toward a wall of plot boards, a ring corridor and a courtyard behind.
// What is changed here is the palette, the boards and everything printed on a wall —
// a survey operations floor is warm lamps over olive grey, not cold blue-grey.
//
...
// The campaign's own length, rather than a literal. Mission Control's board read
// "SHIFT 1 / 15" because that game has fifteen days; a three-level campaign under
// the same board says 1 / 15 and is lying about itself in the first frame.
...
const boardScreens = [];
/** The three plots at the front of the room, which are not the status board. */
const plotBoards = [];
...
function buildMaterials(){
  M.wall = new THREE.MeshStandardMaterial({ color: 0x3a3a33, roughness: 0.92, envMapIntensity: 0.35 });
  M.floor = new THREE.MeshStandardMaterial({ color: 0x2e2b25, roughness: 0.86, envMapIntensity: 0.3 });
  M.riser = new THREE.MeshStandardMaterial({ color: 0x1f1c17, roughness: 0.9 });
  M.desk = new THREE.MeshStandardMaterial({ color: 0x51483b, roughness: 0.62, metalness: 0.2, envMapIntensity: 0.5 });
  M.top = new THREE.MeshStandardMaterial({ color: 0xa49a85, roughness: 0.5, metalness: 0.25 });
  M.frame = new THREE.MeshStandardMaterial({ color: 0x6f6454, roughness: 0.45, metalness: 0.5 });
  M.glass = new THREE.MeshPhysicalMaterial({
    color: 0xb9c1b0, roughness: 0.08, metalness: 0, transmission: 0.82,
    transparent: true, opacity: 0.35, thickness: 0.04,
  });
  M.ceilingPanel = new THREE.MeshStandardMaterial({
    color: 0xf1e7d2, emissive: 0xe9dabe, emissiveIntensity: 0.55, roughness: 0.7,
  });
  ...
  M.corridorWall = new THREE.MeshStandardMaterial({ color: 0x4a463b, roughness: 0.9, envMapIntensity: 0.4 });
  M.corridorFloor = new THREE.MeshStandardMaterial({ color: 0x36312a, roughness: 0.78, envMapIntensity: 0.35 });
  M.paving = new THREE.MeshStandardMaterial({ color: 0x3a3630, roughness: 0.95, envMapIntensity: 0.3 });
  ...
  M.window = new THREE.MeshPhysicalMaterial({
    color: 0xd8e0d2, roughness: 0.03, metalness: 0, transmission: 0.95,
  ...
  M.corridorPanel = new THREE.MeshStandardMaterial({
    color: 0xf2ece0, emissive: 0xeadfc8, emissiveIntensity: 0.95, roughness: 0.65 });
}
 ...
 */
/**
 * Fit out one of the wing rooms.
 *
 * These are the rooms behind the analysis floor — spectroscopy and calibration
 * on the west side, the publication room on the east — each twenty metres long.
 * The vocabulary is the building's own: equipment racks, cable trays, benches,
...
function furnishWorkroom(spec, { xIn, xOut, y, cz }){
  ...
  const NOTICES = {
    SPEC: [
      { style: 'warning', tag: 'ARC LAMP', heading: 'Take a comparison arc every setup',
        accent: '#a33f2f',
        body: 'The wavelength scale moves when the grating is touched and when the '
          + 'building cools. An arc taken afterwards is not the arc for these frames.' },
      { style: 'list', tag: 'REST WAVELENGTHS', heading: 'The lines we work from',
        accent: '#3f6f8f',
        items: [['Si II', '635.5 nm'], ['Ca II H', '396.8 nm'], ['Ca II K', '393.4 nm'],
          ['Na I D', '589.3 nm'], ['O I', '777.4 nm']],
        body: 'Redshift is measured against the rest wavelength, never against last week\'s frame.' },
      { style: 'banner', tag: 'A TYPE IS A CLAIM', heading: 'Say which line you saw',
        accent: '#5b6a72',
        body: 'Silicon at 615 nm in the observed frame is what makes it a Type Ia. '
          + 'A light curve that looks right is not a classification.' },
      { style: 'chart', tag: 'HOST GALAXY', heading: 'Redshifts logged this run',
        accent: '#5b6a72', body: 'Host redshift is preferred. The supernova\'s own lines are broad.' },
      { style: 'sticky', tag: 'NOTE', heading: 'Colour is not distance',
        accent: '#8a5a2b',
        body: 'A reddened supernova and a distant one are both faint. Only the colour tells them apart.' },
    ],
    PRESS: [
      { style: 'banner', tag: 'WHAT WE CLAIM', heading: 'The measurement, then the model',
        accent: '#8a5a2b',
        body: 'Write what the data show before you write what explains it. The two go in '
          + 'separate sentences, and the second one is the one that will be wrong first.' },
      { style: 'list', tag: 'BEFORE IT GOES OUT', heading: 'Signed off by', accent: '#3f6f8f',
        items: [['Photometry', 'the light curves'], ['Spectroscopy', 'the types'],
          ['Calibration', 'the zero points'], ['Cosmology', 'the fit'],
          ['The other team', 'not required, but read it']],
        body: 'A result nobody outside the room has argued with is not finished.' },
      { style: 'warning', tag: 'EMBARGO', heading: 'Nothing to a reporter before the referee',
        accent: '#a33f2f',
        body: 'Not a number, not a hint, not a photograph of a plot on a wall.' },
      { style: 'photo', tag: 'THE OTHER TEAM', heading: 'Same result, different instrument',
        accent: '#5b6a72', body: 'Two independent groups agreeing is the strongest line in the paper.' },
    ],
  };
 ...
 * the control room and the wings — so what is on its walls is the whole programme's
 * business rather than one desk's: the analysis rules, tonight's telescopes, the
 * blind-analysis standing order, and who may say anything to a journalist.
 *
...
function furnishRingSigns(){
  ...
  const SHEETS = [
    { style: 'banner', tag: 'HOUSE RULE 3', heading: 'Disagreeing measurements',
      accent: '#8a5a2b',
      body: 'Two instruments that disagree are not one instrument that is wrong. '
        + 'Find what they share before you decide which to believe.' },
    { style: 'list', tag: 'TONIGHT', heading: 'Telescopes on our targets', accent: '#5b6a72',
      items: [['4 m, imaging', 'from 21:40'], ['8 m, spectra', 'from 23:10'],
        ['Space telescope', 'orbit 4 and 5'], ['Weather', 'clear, seeing 0.9"']],
      body: 'Follow-up is confirmed each afternoon by the scheduling desk.' },
    { style: 'warning', tag: 'THE ZERO POINT', heading: 'One calibration, one night',
      accent: '#a33f2f',
      body: 'Do not carry a zero point across a night. If you did not measure it, '
        + 'do not assume it — every magnitude in the sample rests on it.' },
    { style: 'banner', tag: 'BLIND ANALYSIS', heading: 'The cosmology stays sealed',
      accent: '#8a5a2b',
      body: 'No fitted parameter goes on this floor until the light curves and the '
        + 'calibration are frozen. This includes what you tell each other at lunch.' },
    { style: 'chart', tag: 'SAMPLE', heading: 'Standardised events, this campaign',
      accent: '#5b6a72', body: 'Low redshift and high redshift are counted separately, always.' },
    { style: 'list', tag: 'IF YOU ARE UNSURE', heading: 'Say so, in this order',
      accent: '#3f6f8f',
      items: [['Your own desk', 'first'], ['The room you got it from', 'second'],
        ['The cosmology console', 'always'], ['A reporter', 'never']],
      body: 'An analyst who says "I do not know yet" has done their job.' },
    { style: 'photo', tag: 'THE FIRST SAMPLE', heading: 'Seven events, two years ago',
      accent: '#5b6a72', body: 'Same desks, same plot boards, and not enough supernovae to say anything.' },
    ...
    { style: 'warning', tag: 'DRY RUN', heading: 'When the amber light is on',
      accent: '#8a5a2b',
      body: 'The sample on the boards is simulated, including the parts that look real. '
        + 'Say SIM before every number.' },
  ];
 ...
 */
/**
 * The centre's seal, on the back wall of the analysis floor.
 *
...
function paintControlRoomSeal(){
  ...
  for(const s2 of [-1, 1]){
    paintMural({
      ...
      w: size, h: size,
      kind: 'seal',
      text: { heading: 'CERRO VELA OBSERVATORY', body: 'SURVEY OPERATIONS' },
      seed: `seal-${s2}`,
    });
  }
}

function paintChainWall(){
  const HW = B.halfWidth;
  const [, n1] = B.northLeg;
  const y = ENTRY_Y;
  const T = 0.18;
...
/**
 * The sample, once, as numbers rather than as scenery.
 *
 * Forty-two events on a fixed seed. Fixed because the boards repaint whenever
 * the state changes and a fresh draw each time would make the diagram twitch;
 * and because the residual plot and the Hubble diagram have to be *the same
 * sample*, which they are not if each generates its own.
 *
 * Everything is in 0..1 of its own plotting box, which is the contract
 * `instrumentScreen`'s plot face takes: the caller states a relationship and
 * the board decides how many pixels high it is.
 */
const SAMPLE = (() => {
  // A small deterministic generator. Math.random would reseed on every repaint.
  let s = 20250823;
  const rnd = () => ((s = (s * 1103515245 + 12345) & 0x7fffffff) / 0x7fffffff);
  // The two histories. Distance modulus rises with redshift either way; the flat
  // model with a repulsive term sits about a quarter of a magnitude above the
  // matter-only one over this range, and that gap is the whole result.
  const line = (lift) => Array.from({ length: 40 }, (_, i) => {
    const z = i / 39;
    return [z, 0.10 + 0.72 * Math.sqrt(z) + lift * z];
  });
  const matter = line(0);
  const lambda = line(0.16);
  const events = Array.from({ length: 42 }, () => {
    const z = 0.04 + rnd() * 0.94;
    const on = 0.10 + 0.72 * Math.sqrt(z) + 0.16 * z;
    return { z, mu: on + (rnd() - 0.5) * 0.10 };
  }).sort((a, b) => a.z - b.z);
  return {
    matter, lambda,
    points: events.map((e) => [e.z, e.mu]),
    // The same events against the matter-only model: observed minus predicted,
    // drawn about a zero line in the middle of the box.
    residuals: events.map((e) => {
      const pred = 0.10 + 0.72 * Math.sqrt(e.z);
      return [e.z, 0.5 + (e.mu - pred) * 1.6];
    }),
    // Three light curves with the same peak and different decline rates, which
    // is the one thing the standardisation turns on.
    curves: [1.5, 1.0, 0.68].map((broad, i) => ({
      faint: i !== 1,
      points: Array.from({ length: 44 }, (_, k) => {
        const t2 = k / 43;
        const v = t2 < 0.20 ? (t2 / 0.20) ** 2
          : Math.exp(-(((t2 - 0.20) / (0.34 * broad)) ** 1.1));
        return [t2, 0.08 + 0.86 * v];
      }),
    })),
  };
})();
...
/**
 * What each board is showing, for a given level.
 *
 * The cosmology is sealed until the ruler is agreed, which is the campaign's own
 * rule and the reason the room can be walked into on level 1 without the answer
 * being on the wall. So the diagram is blank on level 1, carries the points
 * against gravity's own prediction on level 2, and only puts the second history
 * up on level 3.
 */
function boardFace(label, level){
  if(label === 'LIGHT CURVES'){
    return { kind: 'plot', title: label, curves: SAMPLE.curves,
      note: 'three decliners, one peak brightness',
      axes: { x: 'days from peak', y: 'brightness' } };
  }
  if(label === 'HUBBLE DIAGRAM'){
    if(level < 2){
      return { kind: 'plot', title: label, note: 'sealed until the ruler is agreed',
        axes: { x: 'redshift', y: 'distance modulus' } };
    }
    const curves = [{ points: SAMPLE.matter, dashed: true, label: 'matter only' }];
    if(level >= 3) curves.push({ points: SAMPLE.lambda, label: 'flat, with a repulsive term' });
    return { kind: 'plot', title: label, curves, points: SAMPLE.points,
      shown: SAMPLE.points.length,
      note: level >= 3 ? 'both histories, one sample' : 'against gravity alone',
      axes: { x: 'redshift', y: 'distance modulus' } };
  }
  // RESIDUALS
  const zero = [{ points: [[0, 0.5], [1, 0.5]], faint: true, label: 'matter only' }];
  if(level < 2){
    return { kind: 'plot', title: label, curves: zero, note: 'nothing plotted yet',
      axes: { x: 'redshift', y: 'observed − model' } };
  }
  return { kind: 'plot', title: label, curves: zero, points: SAMPLE.residuals,
    shown: SAMPLE.residuals.length, status: 'high',
    note: level >= 3 ? 'about a quarter of a magnitude faint, at every redshift'
      : 'faint, and not only at the far end',
    axes: { x: 'redshift', y: 'observed − model' } };
}

/** The wall of plot boards: the only bright thing in the room. */
function buildBoards(){
  // The foot of the boards. See the note on BOARDS: this number and the heights
  // there are one constraint against the room's ceiling.
  const y = 1.15;
  for(const b of BOARDS){
    const screen = instrumentScreen(boardFace(b.label, 1), { w: 512, h: 256 });
    ...
    // Kept apart from `boardScreens`, which is the status board's list: these
    // three are the plots and are set from the level, and pushing them into one
    // array is how three boards ended up printing one progress line each.
    plotBoards.push({ mesh, screen, label: b.label });
  }
}
...
export function initWorld(canvas, activeTheme){
  ...
  const statusScreen = instrumentScreen({ kind: 'panel', title: 'RUN STATUS', rows: [] }, { w: 512, h: 256 });
  ...
  scene.add(status);
  interactables.push({ mesh: status, type: 'board', id: 'BOARD', prompt: 'E — Open the run status board' });
...
export function updateWorldFromState(state, nextStopId = null, pct = () => 0){
  ...
  for(const p of plotBoards) p.screen.set(boardFace(p.label, m));
  for(const b of boardScreens){
    b.screen.set({
      rows: [
        { label: 'LEVEL', value: `${m} / ${MISSIONS.length}`, status: 'normal' },
 ...
 * only clock on this floor is the observing clock. Kept because the contract
 * requires it.
 */
