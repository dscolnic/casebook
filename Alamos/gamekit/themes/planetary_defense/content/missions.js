...
export const MISSIONS = [
  ...
  },
  {
    ...
    "stake": "The pass is tonight, and Fischer has the dish. The radar window is four hours wide, and this geometry does not return for eleven years — after the encounter that everybody is now worried about. Three of the four things the room wants can be got another way. Today you decide what radar is uniquely for and write the sequence. Radar is not a better telescope. A telescope says where something is on the sky. Radar sends a pulse and times the echo, which gives distance directly, and reads the shift in what comes back, which gives how fast the object is closing. Those are exactly what an orbit built from angles is starved of, and it resolves shape at a scale nothing optical here can reach. Tonight the size question closes for good, or it stays open for a decade.",
  ...
  },
  {
    ...
    "stake": "Eight years after the discovery, and the encounter is next week. The impactor struck five years ago; tracking since then puts the object about two Earth radii out, and some five thousand kilometres of that — most of an Earth radius — was bought by the impact. It was on a path nobody would have accepted. Nothing further can be done now, which is as it should be, because everything that could be done was done seven years ago. Today you take the claims one at a time with Garcia, write the public record with Ellery, and decide what stays funded once nobody is frightened. Each claim carries its own strength: what was measured, what was assumed, what stayed open. The population this object came from has not changed. The next one is up there now, unfound, and the survey line that caught this one expires in eleven months.",
