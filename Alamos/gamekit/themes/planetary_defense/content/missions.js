...
export const MISSIONS = [
  ...
  },
  {
    ...
    "stake": "Five days out. A broad dip has appeared near 1.4 microns in the spectrum, and if it is real it changes the composition class the draft circular already names. Today you decide whether that feature belongs to the asteroid, and what would make the claim solid. A spectrum records which colours of light came back and which went missing, and different minerals remove different colours. The trouble is that our own atmosphere removes light too, and water vapour above the telescope absorbs strongly in exactly this region. This is not a label for a catalogue. Density comes from composition, mass comes from density, and so does strength — which decides whether the body holds together when a spacecraft hits it in three years' time. Every model built after today inherits whatever is written today.",
  ...
  },
  {
    ...
    "stake": "The pass is tonight. The radar window is four hours wide, and this geometry does not return for eleven years — after the encounter that everybody is now worried about. Three of the four things the room wants can be got another way. Today you decide what radar is uniquely for and write the sequence. Radar is not a better telescope. A telescope says where something is on the sky. Radar sends a pulse and times the echo, which gives distance directly, and reads the shift in what comes back, which gives how fast the object is closing. Those are exactly what an orbit built from angles is starved of, and it resolves shape at a scale nothing optical here can reach. Tonight the size question closes for good, or it stays open for a decade.",
  ...
  },
  {
    ...
    "stake": "Four months after the pass, and the encounter is seven and a half years out — which sounds like plenty and is not, because a deflection has to be launched years before it arrives. Today you size the nudge and decide how success will be measured. A deflection is a small change in speed applied very early, so the object arrives somewhere else: a centimetre or two per second, multiplied by five years of travel, is a few thousand kilometres of miss. The push comes from the spacecraft's momentum plus the debris thrown off the far side, which can double it. On a weak body it can also break the object apart, and several fragments on the same path is worse than one. So what has to be measured is not what you launched but what actually happened to the orbit.",
  ...
  },
  {
    ...
    "stake": "Nine months after the pass. The launch window is nineteen days wide and the composition result that would say what you are aiming at comes back eight days after it closes. Today you find which subsystem is really setting the limit and decide where to spend margin. Every architecture is held back by one thing first: how much mass the rocket can send, the power aboard, how well the spacecraft can see the target, or how fast it can decide for itself when the signal takes minutes each way. Fixing anything else is decoration. And this is the decision the whole campaign turns on, because lead time is the only resource that cannot be bought back. Miss this window and there is no second attempt and no cheaper version — only telling people where to stand in seven years.",
  ...
  },
  {
    ...
    "stake": "Three years in. The impactor launched two years ago and reaches the asteroid in eleven days, five years before the encounter. The probability has moved twice this month, both times downward, and a draft evacuation plan has been seen by two regional governments who are being asked to fund it now. Today you match what gets done to what is actually known. Preparation should scale with four things: how likely it is, how bad it would be, how much time there is, and how easily each step can be undone. Stockpiles and plans are cheap and reversible. Moving nine million people is neither, and it kills people directly — on the roads, in the hospitals, among the very old. Both mistakes here have a body count, and only one of them would ever be reported as yours.",
  ...
  },
  {
    ...
    "stake": "Eight years after the discovery, and the encounter is next week. The impactor struck five years ago; tracking since then puts the object about two Earth radii out, and some five thousand kilometres of that — most of an Earth radius — was bought by the impact. It was on a path nobody would have accepted. Nothing further can be done now, which is as it should be, because everything that could be done was done seven years ago. Today you take the claims one at a time, write the public record, and decide what stays funded once nobody is frightened. Each claim carries its own strength: what was measured, what was assumed, what stayed open. The population this object came from has not changed. The next one is up there now, unfound, and the survey line that caught this one expires in eleven months.",
