...
// Half-width of a hatch opening. A real one is about this; this one is wider,
// because a player who has to line up to a centimetre to walk through a door
// spends the watch bouncing off bulkheads.
const HATCH_HALF = 0.68;
const HATCH_H = 1.95;
...
export class SubmarineWorld {
  ...
  _buildBulkheadsAndHatches() {
    for (let i = 0; i < LAYOUT.length - 1; i++) {
      ...
      // First word of the compartment name, without the punctuation that comes
      // with it: "Berthing, Mess & Medical" was signed "BERTHING," comma and all.
      const sign = this.props.sign(LAYOUT[i + 1].name.split(' ')[0].replace(/[,&]/g, ''));
  ...
  _furnishCompartments() {
    for (const c of LAYOUT) {
      ...
      for (const r of matRects) {
        ...
        // Deck matting is rubber, so it is rough. It used to use the generic
        // emissive material at 0.4 roughness, and under a brighter compartment
        // light it blew out to a sheet of white across the passage.
        const mattingMat = this.mat.emissive(new THREE.Color(c.color).getHex(), 0.02).clone();
        mattingMat.roughness = 0.96;
        mattingMat.metalness = 0.0;
        const mat = new THREE.Mesh(new THREE.PlaneGeometry(w, d), mattingMat);
