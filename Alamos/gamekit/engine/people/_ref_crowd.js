...
function nameplate(ch, division){
  const cvs = document.createElement('canvas');
  cvs.width = 512; cvs.height = 96;
  const ctx = cvs.getContext('2d');
  ctx.fillStyle = 'rgba(250,250,247,0.97)'; ctx.fillRect(0, 0, 512, 96);
  ctx.fillStyle = '#25506b'; ctx.fillRect(0, 0, 512, 4);
  ctx.fillStyle = '#1b1e22';
  ctx.font = '800 25px Inter, Helvetica, Arial, sans-serif';
  ctx.textAlign = 'center'; ctx.fillText(ch.name, 256, 40);
  ctx.fillStyle = '#5b6068';
  ctx.font = '600 17px Inter, Helvetica, Arial, sans-serif';
  ctx.fillText(ch.role.length > 44 ? ch.role.slice(0, 44) + '…' : ch.role, 256, 70);
  const tex = new THREE.CanvasTexture(cvs);
  tex.colorSpace = THREE.SRGBColorSpace;
  tex.anisotropy = 8;
  const plate = new THREE.Mesh(
    new THREE.PlaneGeometry(1.35, 0.25),
...
export function spawnNPCs(count = 40){
  ...
  roster.forEach((ch, i) => {
    ...
    const plate = nameplate(ch, division);
    plate.position.set(0, seat ? 1.55 : 1.95, 0);
    body.add(plate);
...
function stepWalker(n, delta){
  ...
  const stride = 0.74;
  n.bobPhase += delta * (n.speed / stride) * Math.PI * 2;
  const swing = Math.min(0.5, Math.max(0.14, n.speed * 0.40));
  const s1 = Math.sin(n.bobPhase) * swing, s2 = -s1;
  n.body.userData.limbs.forEach(l => {
    if(l.userData.isLeg){
      const sw = l.userData.side === -1 ? s1 : s2;
      l.rotation.x = sw;
      // knee flexes on the forward swing only, as a real stride does
      if(l.userData.knee) l.userData.knee.rotation.x = Math.max(0, -sw) * 0.9;
    } else {
      l.rotation.x = (l.userData.side === -1 ? s2 : s1) * 0.62;
    }
  });
  if(n.body.userData.torso) n.body.userData.torso.rotation.y = -Math.sin(n.bobPhase) * 0.10;
  if(n.body.userData.head) n.body.userData.head.rotation.y = Math.sin(n.bobPhase) * 0.06;
  n.body.position.set(n.pos.x, Math.abs(Math.sin(n.bobPhase)) * 0.03, n.pos.z);
