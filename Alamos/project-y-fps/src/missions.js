const MISSION_DEFS=[
  {
    ...
    "stake": "April 1943, and the site is three weeks old. Five divisions, one mesa, and three different ways of writing the same nucleus on three different blackboards. The laboratory director, J. Robert Oppenheimer, has scheduled a colloquium every week for exactly this reason: nothing here can be coordinated until everybody describes matter the same way. Today you settle three things. What a nuclide symbol actually specifies. Why two isotopes of an element act alike in a beaker and differently in a counter. And how radiation becomes a number an instrument can report. The second follows from the first: chemistry is decided by the electrons, and the electrons are set by the protons. The third is how you know any of it — radiation knocks electrons off atoms in a detector, and that tiny charge becomes a pulse. Every measurement made here for the next two years rests on these three."
  },
  {
  ...
  },
  {
    ...
    "stake": "September 1944, and compression is now the whole programme. Lilli Hornig, the chemist casting the metal at the centre of it, has found that it changes density under conditions the design will actually meet. Nobody wanted that, and the mathematician John von Neumann wants the shock tables redone with it in. Today you work with the relationship between pressure, volume and temperature, read what a phase diagram is telling you, and understand what shaping a detonation wave achieves. An equation of state is that relationship written down. Squeeze something, and how much it gives depends on how hot it is and what it is made of. A phase diagram says which arrangement of atoms exists under which conditions. Different arrangements have different densities, so a material can rearrange itself part way through and behave like something else. None of the three can be assumed from the other two. And shaping a wave that spreads outward into one that converges inward is von Neumann's problem before it is the explosives division's, under George Kistiakowsky: it is done in numbers first, and only then does X Division cut anything."
  },
  {
