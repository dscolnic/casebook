...
export function initPlayer(canvas, scene, renderer){
  // far must clear the outermost ridge ring (radius 660) plus the player's reach.
  // near is raised to 0.25 to keep depth precision where the geometry is dense.
  camera = new THREE.PerspectiveCamera(68, window.innerWidth/window.innerHeight, 0.25, 900);
...
export function updatePlayer(delta){
  ...
  // Trees, poles and barrels are cylinders — a squared distance test, so the
  // several hundred of them cost far less than adding them all to `colliders`.
  const hitsSoft=(x,z)=>{
  ...
  // keep inside world bounds — the mesa top stays flat out past this radius
