...
/**
 * The panel shown *before* the question. It must carry the scene — the clues the
 * player reasons from — and must never carry `takeaway` or `why`, which state
 * the answer. Showing the takeaway here made the first question unanswerable
 * while simultaneously giving the answer away.
 */
function introBody(lesson){
  // Who and where. The scene itself is rendered by the story block below, so
  // repeating it here printed the same paragraph twice.
  const cast = lesson.game && lesson.game.setup ? lesson.game.setup : '';
  return cast
    ? `<div style="font-size:.74rem;color:#666158;margin:4px 0 0;font-weight:700">${esc(cast)}</div>`
    : '';
}
...
function reasoningHTML(ch, lesson, solution, whyText){
  ...
  if(ch.type==='Sequence') detail=`<ol class="reasonList">${ch.order.map((idx,n)=>`<li><b>${n+1}.</b> ${esc(ch.cards[idx])}</li>`).join('')}</ol>`;
    ...
    // The trailing "Why:" line below already carries this, so leaving it here
    // printed the explanation twice.
    detail='';
  }
  const takeaway=lesson.takeaway?`<div class="answerScienceLead">${esc(lesson.takeaway)}</div>`:'';
  // Why each tempting answer fails — the book writes one line per distractor,
  // and they are the most useful part of the explanation for a learner.
  const rebuttals = (ch.rebuttals||[]).length
    ? `<ul style="margin:8px 0 0;padding-left:18px;font-size:.8rem;color:#4e4a43">${
        ch.rebuttals.map(r=>`<li style="margin:3px 0">${esc(r)}</li>`).join('')}</ul>`
    : '';
  const whyLine = whyText ? `<p><b>Why:</b> ${esc(whyText)}</p>` : '';
  // The answer itself is already shown by the compare boxes above, so repeating
  // it here printed "Correct answer" twice.
  return `${detail}${takeaway}${whyLine}${rebuttals}`;
}
...
function bindTriage(){
  ...
  check.onclick=()=>{
    ...
    activeChallenge.userAnswer = chosen != null && ch.choices[chosen] ? ch.choices[chosen] : '(no answer)';
    finishVisit(ok);
  };
}
function bindDiagnosis(){
  ...
  btns.forEach(b=> b.addEventListener('click', ()=>{
    btns.forEach(x=> x.classList.remove('selected'));
    b.classList.add('selected'); chosen=+b.dataset.diagnosis; if(check) check.disabled=false;
  }));
  if(!check) return;
  check.onclick=()=>{
    const ch=activeChallenge.ch;
    ...
    activeChallenge.userAnswer = picked || '(no answer)';
    const ok = picked===correctStr;
    finishVisit(ok);
  };
}
...
function finishVisit(ok){
  ...
  const comparison = ok
    ? `<div class="answerCompareBox correct"><b>Your answer</b>${esc(activeChallenge.userAnswer||solution)}</div>`
