...
function fitCorridor(){
  ...
  // Parked against a side wall means running *parallel* to it — long axis along
  // z. Rotating these by 90° laid them across the corridor: the gurney alone
  // spanned 2 m of a 3.6 m corridor and swallowed the player's spawn point, so
  // every move was blocked and the game looked fine but would not walk.
  supplyCart(-HW + 0.42, 9.0, 0.04);
  supplyCart(HW - 0.42, 21.5, -0.03, M.plasticBlue);
  supplyCart(-HW + 0.42, 35.0, 0.05);
  gurney(HW - 0.62, 14.5, 0.03, { blanket: false });
  gurney(-HW + 0.62, 29.0, -0.02);
  wheelchair(HW - 0.5, 4.5, Math.PI + 0.3);
  wheelchair(-HW + 0.5, 43.0, -0.2);
  workstationCart(HW - 0.5, 31.0, Math.PI);
  ivPole(-HW + 0.4, 17.5);
  ivPole(HW - 0.4, 37.0);
