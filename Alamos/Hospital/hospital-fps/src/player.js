...
export function initPlayer(canvas, scene, renderer){
  // Narrower than the outdoor build: a 72° field distorts badly in a corridor.
  camera = new THREE.PerspectiveCamera(66, window.innerWidth/window.innerHeight, 0.08, 160);
...
/**
 * Is this spot free? Used to keep the player from being placed inside geometry.
 */
export function positionBlocked(x, z, radius = 0.45){
  const b = new THREE.Box3().setFromCenterAndSize(
    new THREE.Vector3(x, playerHeight / 2, z),
    new THREE.Vector3(radius * 2, playerHeight, radius * 2));
...
/**
 * If a spot is inside geometry, spiral outward for the nearest free one.
 * Without this, a prop placed over the spawn point silently welds the player in
 * place: the move is blocked, and both slide-along-axis fallbacks are blocked
 * too, so the game renders perfectly and simply will not walk.
 */
function nearestFreeSpot(x, z){
  if(!positionBlocked(x, z)) return { x, z };
  for(let r = 0.4; r <= 6; r += 0.4){
    for(let i = 0; i < 16; i++){
      const a = (i / 16) * Math.PI * 2;
      const nx = x + Math.cos(a) * r, nz = z + Math.sin(a) * r;
      if(!positionBlocked(nx, nz)){
        console.warn(`[player] spawn (${x.toFixed(2)}, ${z.toFixed(2)}) was inside ` +
          `geometry; moved to (${nx.toFixed(2)}, ${nz.toFixed(2)}). Check prop placement.`);
        return { x: nx, z: nz };
      }
    }
  }
  console.error(`[player] no free spot within 6 m of (${x.toFixed(2)}, ${z.toFixed(2)}) — ` +
...
export function teleport(pos, yaw){
  const free = nearestFreeSpot(pos.x, pos.z);
  controls.getObject().position.set(free.x, playerHeight, free.z);
