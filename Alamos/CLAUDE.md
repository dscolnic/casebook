...
| `gamekit/PLACEMENT_PASS.md` | **Built on `redsand`; carries a per-campaign runbook.** Where each question is *asked* — decisions to people, calculations to rooms, anything the player *operates* to the equipment it operates — plus more enterable places, a plant that starts unfinished, and a world that opens as the campaign reaches it. Carries the rule the growth half turns on: **seal doors, never ground**. The machinery is engine-wide already; per campaign it is a `fixtures.js`, `at:` on the lessons, `enter:` on the closed buildings, and a look at the renders. Pairs with `BRIEFING_PASS.md`. |
| `gamekit/BRIEFING_PASS.md` | **Built on `redsand`; carries a per-campaign procedure.** What the cards *say*: a two-sentence stake, a reason under each objective saying why *today*, no delivery line, no word floors, the HUD down to three readings. Carries the rule the pass turned up — **a delivery that has to be glossed on the opening card is usually the wrong delivery**, and Red Sand's was a document about the fuel rather than the fuel. Pairs with `PLACEMENT_PASS.md`. |
